...
export const isContentSeen = (mediaId: string) => {
  const storage = LocalStorageService.getItem<LocalStorageSeenItem[]>(LocalStorageKeys.VIEWED_CONTENT)

  return storage?.some(item => item.id === mediaId)
}

export const checkIfContentHasBeenSeenBefore = (id: string) => {
  const items = LocalStorageService.getItem<LocalStorageSeenItem[]>(LocalStorageKeys.VIEWED_CONTENT) ?? []
  ...
  if (!item) {
    ...
    LocalStorageService.saveItem(LocalStorageKeys.VIEWED_CONTENT, [...items ?? [], newItem])
    return newItem
  }
