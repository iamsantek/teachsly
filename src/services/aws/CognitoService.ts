import { ListUsersCommand, CognitoIdentityProviderClient, AdminAddUserToGroupCommand, ListUsersInGroupCommand, ListGroupsCommand, AdminUpdateUserAttributesCommand, AdminCreateUserCommand, AttributeType } from "@aws-sdk/client-cognito-identity-provider";
...
class CognitoService {
    ...
    constructor() {
        this.cognitoIdentityProviderClient = this.createCognitoIdentityProviderClient();
    }
    ...
    public assignUserToCognitoGroup = async (userId: string, groups: string[]) => {
        try {
            ...
            const adminAddUserToGroupCommandPromises = groups.map(groupName => {
                ...
                return this.cognitoIdentityProviderClient.send(adminAddUserToGroupCommand);
            })
    ...
    public getCognitoUsersByGroupName = async (groupName: string) => {
        try {
            ...
            const listUsersInGroupCommandResponse = await this.cognitoIdentityProviderClient.send(listUsersInGroupCommand);
    ...
    public getCognitoGroups = async () => {
        try {
            ...
            const listGroupsCommandResponse = await cognitoIdentityProviderClient.send(listGroupsCommand);

            return listGroupsCommandResponse.Groups?.map(group => group.GroupName).filter(Boolean);
        } catch (error) {
            Logger.log(
                LogLevel.ERROR,
                LogTypes.AuthService,
    ...
    public createCognitoUser = async (username: string, name: string, type: UserTypes) => {
