...
export const CourseCardButtons = ({
  course: { externalId, virtualClassLink },
}: Props) => {
  ...
  return (
    <Box display="flex" flexDirection={"column"} gap={2}>
      <TooltipHelper label={translate("SEE_CONTENT")}>
        <Box as={NavLink} to={generateMediaByCourseRoute(externalId)}>
          <IconButton
            icon={<AiFillFolder />}
            isRound
            size="md"
      ...
      {virtualClassLink && (
        <TooltipHelper label={translate("GO_TO_VIRTUAL_CLASS")}>
          <Box as={"a"} href={virtualClassLink} target="_blank">
            <IconButton
              icon={<AiOutlineLink />}
              isRound
              size="md"
      ...
      {hasTeacherRole && (
        <TooltipHelper label={translate("GO_TO_STUDENTS_LIST")}>
          <Box as={NavLink} to={generateStudentsByCourseRoute(externalId)}>
            <IconButton
