...
const UserCRUDModal = ({
  ...
  const [showDisableUserConfirmation, setShowDisableUserConfirmation] =
  ...
  const newUserButtonName =
    userType === UserTypes.STUDENT
      ? "CREATE_STUDENT_BUTTON"
      : "CREATE_TEACHER_BUTTON";
  const editUserButtonName =
    userType === UserTypes.STUDENT
      ? "EDIT_STUDENT_BUTTON"
      : "EDIT_TEACHER_BUTTON";
  ...
  useEffect(() => {
    ...
    const englishLevel = mapSingleValueToMultiSelectOption(
      userToUpdate.englishLevel || ""
    );
  ...
  const onDeleteUser = async () => {
    ...
    const deletedUser = await UserService.deleteUser(
      ...
      userToUpdate?.cognitoId as string
    );
  ...
  const updateUser = async (user: UpdateUserInput) => {
    ...
    const shouldUpdateGroups = updatedFields.some(
      (field) => field === "groups" || field === "englishLevel"
    );
    const currentGroups = [
      ...(userToUpdate?.groups as string[]),
      userToUpdate?.englishLevel as string,
    ];
  ...
  const modalTitle = () => {
    const newUserModalTitle = translate(
      userType === UserTypes.STUDENT
        ? "CREATE_STUDENT_MODAL_TITLE"
        : "CREATE_TEACHER_MODAL_TITLE"
    );

    return userId
      ? `${translate("EDITING")} '${userToUpdate?.name}'`
      : newUserModalTitle;
  };
  ...
  const toggleAccountStatus = async () => {
    ...
    const updateUserResponse = await UserService.updateUser(
      disabledUser as UpdateUserInput
    );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="4xl">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader textStyle={"paragraph"}>{modalTitle()}</ModalHeader>
          ...
          <FormProvider {...formControls}>
            <form onSubmit={handleSubmit(onSubmit)}>
              <ModalBody>
                ...
                <ConfirmationDialog
                  ...
                  title={
                    isDisabledUser
                      ? "ACTIVE_USER_BUTTON"
                      : "DEACTIVATED_USER_BUTTON"
                  }
                  description={
                    isDisabledUser
                      ? "ACTIVE_USER_DESCRIPTION"
                      : "DEACTIVATED_USER_DESCRIPTION"
                  }
                  confirmButtonText={
                    isDisabledUser
                      ? "ACTIVE_USER_BUTTON"
              ...
              <ModalFooter
                isLoading={isLoading}
                sendButtonText={
                  userToUpdate ? editUserButtonName : newUserButtonName
                }
                onClose={onClose}
              ></ModalFooter>
            </form>
