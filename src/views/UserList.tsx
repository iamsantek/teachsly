...
const UserList = ({ listType }: Props) => {
  ...
  const [viewModalVisibility, setViewModalVisibility] =
    useState<boolean>(false);
  const [currentCourseFilter, setCurrentCourseFilter] = useState<string>("");

  const {
    context: { user: loggedUser, courses },
  ...
  const fetchUsers = useCallback(async () => {
    ...
    if (hasTeacherRole) {
      ...
      filteredUsers = users.filter((user) =>
        user.groups.filter((group) => loggedUser?.groups.includes(group))
      );
    }
  ...
  const newUserButtonName = translate(
    listType === UserTypes.STUDENT
  ...
  const onChangeCourseFilter = (courseFilter: string) => {
    ...
    const filterUsers = users.filter((user) =>
      user.groups.includes(courseFilter)
    );
      ...
      <Stack spacing={4}>
        ...
        <Flex alignItems="center" gap={5} maxWidth={["100%", "40%"]}>
          ...
          >
            ...
            {courses.map((course) => (
              <option key={course.externalId} value={course.externalId}>
        ...
        <Stack>
          ...
          <NoContentPlaceholder
            show={usersDisplayed.length === 0 && !isLoadingNewPage}
          />
          <Placeholder
