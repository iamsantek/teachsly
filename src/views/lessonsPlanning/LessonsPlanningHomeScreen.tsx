...
export const LessonsPlanningHomeScreen = () => {
  ...
  const fetchLessonPlans = useCallback(async () => {
    ...
    if (lessonPlans?.listLessonPlans?.nextToken) {
      dispatch({ type: FetchType.LESSONS, payload: lessonPlans.listLessonPlans.nextToken })
    }
  ...
  const generateLessonPlanningByExams = useCallback(() => {
    const lessonPlannings: CreateLessonPlanInput[] = exams?.map(exam => {
      const examAttempt = examAttempts.find(attempt => attempt.examName === exam?.title && attempt.score)
      const pendingCorrection = examAttempt?.isCompleted && !examAttempt.correctedBy
      const { badgeColor, badgeText } = getBadgeWithExamAttempt(exam, examAttempt as ExamAttempt)
  ...
  const fetchExamAttempts = useCallback(async () => {
    ...
    if (examAttempts?.listExamAttempts?.nextToken && examAttempts.listExamAttempts.items.length !== 0) {
      dispatch({ type: FetchType.EXAM_ATTEMPTS, payload: examAttempts.listExamAttempts.nextToken })
    } else {
  ...
  const fetchMedias = useCallback(async () => {
    ...
    const newLessonPlans: CreateLessonPlanInput[] = medias?.listMedia?.items?.map(media => {
      return {
        id: media?.id,
        title: media?.title,
    ...
    setLessons(lessonPlans => lessonPlans.concat(newLessonPlans as LessonPlanningItem[]))
  ...
  const onDelete = async (lessonPlan: LessonPlan) => {
    ...
    if (deletedLessonPlan) {
      const newLessons = lessons.filter(lesson => lesson.id !== lessonPlan.id)
