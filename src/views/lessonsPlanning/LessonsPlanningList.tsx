...
export const LessonsPlanningList = ({ lessons, onCreate, onDelete, onUpdate, isModalOpen, onCloseModal, lessonToUpdate, onUpdateSuccess }: Props) => {
  ...
  const LessonDate = ({ date }: { date: string }) => {
    ...
    return <Flex flexDirection='column' zIndex={11}>
      <Text fontSize={10} fontWeight='bold' textAlign={'center'}> {day}</Text >
      <Text fontSize={10} fontWeight='bold' textAlign={'center'}> {month}</Text >
    </Flex>
  }
  ...
  const onView = async (lesson: LessonPlanningItem) => {
    switch (lesson.type) {
      case LessonPlanningType.MEDIA:
        ...
        const mediaLink = await StorageService.getSignedUrl(lesson.media as string)
              ...
              return (
                <Box>
                  <ContentLine
                    ...
                    noBorder={true}
                    onDelete={hasEditPermission && isLessonPlan ? () => onDelete(lesson) : undefined}
                    onEdit={hasEditPermission && isLessonPlan ? () => onUpdate(lesson) : undefined}
                  ...
                  >
                    <CommonContentLineTitle
                      id={lesson.id}
                      title={lesson.title}
                      header={<Badge variant='subtle' color='gray.900' colorScheme={contentLineColor[lesson.type ?? LessonPlanningType.OTHER]} >{lesson.type}</Badge>}
                    >
                      <Text color="gray.600" noOfLines={3}>
                        {lesson.content}
                      </Text>
                      <Flex gap={3}>
                        {lesson.renderElement}
                      </Flex>
                    </CommonContentLineTitle>
                  </ContentLine>
                </Box>
