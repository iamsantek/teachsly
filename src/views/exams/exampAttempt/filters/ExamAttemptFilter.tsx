...
export const ExamAttemptFilter = ({
  studentNameFilter,
  activeStatusFilter,
  activeNameFilter,
  examAttempts,
  onChangeStatusFilter,
  onChangeExamNameFilter,
  onChangeStudentNameFilter,
}: Props) => {
  ...
  return (
    <Flex alignItems="center" gap={5}>
      ...
      <Select
        ...
        onChange={(e) =>
          onChangeStatusFilter(e.target.value as IExamAttemptFilter)
        }
      >
        <option value={IExamAttemptFilter.ALL}>{translate("ALL")}</option>
        <option value={IExamAttemptFilter.COMPLETED}>
          {translate("FINISHED_EXAMS")}
        </option>
        <option value={IExamAttemptFilter.NOT_COMPLETED}>
          {translate("NOT_FINISHED_EXAMS")}
        </option>
        <option value={IExamAttemptFilter.NOT_CORRECTED}>
          {translate("EXAMS_NOT_CORRECTED")}
        </option>
      </Select>
      <MdLibraryBooks size={70} />
      <Select
        value={activeNameFilter}
        onChange={(e) => onChangeExamNameFilter(e.target.value)}
      >
        <option value={IExamAttemptFilter.ALL}>{translate("ALL")}</option>
