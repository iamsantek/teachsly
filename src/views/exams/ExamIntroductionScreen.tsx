...
export const ExamIntroductionScreen = () => {
  ...
  const { seconds, minutes, hours, days, reset } = useTimer(
    countdown,
    handleTimerFinish
  );
  ...
  const fetchExam = useCallback(
    async (examId: string) => {
      ...
      const examAttemptPromise = ExamService.getExamAttemptByExternalId(
        examId,
      ...
      const countdown = Math.abs(
        dayjs().diff(exam?.getExam?.deadline, "seconds")
      );
  ...
  const examDuration = useMemo(
    () => calculateExamDurationInMinutes(exam as ExamForm),
    [exam]
  );
  ...
  const onStart = async () => {
    ...
    window.location.href = `https://${
      exam.type === ExamType.HOMEWORK ? "homework" : "exams"
    }.${GeneralInformation.DOMAIN}/${
      examAttempt.createExamAttempt?.externalId
    }`;
  };
  ...
  return (
    <Container centerContent marginY={10}>
      <Stack spacing={6}>
        ...
        <Text textAlign="justify">{translate("EXAM_INTRO_WORDING_1")}</Text>
        <Text textAlign="justify">
          {translate(
            isRetakeAllowed
              ? "EXAM_INTRO_WORDING_2_WITH_RETAKE"
